...
// `figureMetrics` moved to the LEAF `figureMetrics.mjs` (inventory 5.25) so `sheetsPack.mjs` can
// reach it without importing this module back. Re-exported because callers import it from here.
export { figureMetrics } from './figureMetrics.mjs';
...
/**
 * `frameLifts` and `packStrip` — packing keyed cells into one uniform-cell strip, with the
 * per-frame vertical lift that keeps every sheet on a single baseline — live in `sheetsPack.mjs`,
 * split out to keep this file under the 400-line limit. Re-exported here because
 * `build-assets.mjs` and the `sheet-packing*.test.ts` files import both by name from
 * `'./sheets.mjs'` and those import sites are not being changed by this split.
 */
export { frameLifts, packStrip } from './sheetsPack.mjs';
