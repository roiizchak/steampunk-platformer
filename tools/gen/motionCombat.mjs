...
import { poseSpan } from './motionClauses.mjs';
/**
 * 🔴 The scavenger's five records live in their own file since 2026-08-15, taking this one from
 * **426 lines to under the limit**. The split was blocked for a session by a stated reason that a
 * gate owner showed was removable: this file used to import `poseSpan` from `motion.mjs`, closing a
 * cycle whose failure mode is a SILENTLY truncated `VIDEO_MOTIONS`. `poseSpan` moved to the leaf
 * `motionClauses.mjs` in the same commit, so **there is no cycle left to deepen** — verified by
 * importing this module first and reading a complete 17-entry table, the exact order that used to
 * truncate it.
 */
import { SCAVENGER_MOTIONS } from './motionCombatScavenger.mjs';
import {
  DEBRIS_MARGIN,
  DISCHARGE_MARGIN,
  FRAME_MARGIN,
