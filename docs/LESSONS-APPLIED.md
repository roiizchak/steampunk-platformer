...
|---|---|---|---|
| **A1** | **PowerShell vs. the `genmedia` CLI.** The vault's rule cost a credit: drive a `.cmd` CLI from a shell, never from a subprocess API — `cmd.exe` quoting mangles multi-line `--prompt` and **the job silently succeeds at the wrong aspect ratio**. Our environment is PowerShell-primary. Verify how `genmedia` resolves and how a multi-line prompt survives our invocation path; prove it by reading returned pixel dimensions off the job record. **The failure mode to fear is not a crash — it is a successful run with dropped arguments.** | Gate 3 | `10 - Evidence/Art & Audio Pipeline/Drive a .cmd CLI from a shell, never from a subprocess API.md` (costly); `10 - Evidence/Deployment & Tooling/Resolving an npm CLI on Windows needs PATHEXT first and a shell for .cmd.md` (costly) |
| **A2** | **The prompting evidence is from a different service.** The vault states plainly that model behaviour dates fast and its prompting findings must be re-verified against the model actually used. The **method** transfers; the **facts** are Higgsfield-era. Specifically re-measure on fal.ai: *does the model emit real alpha?* — if yes, the entire chroma-key apparatus (and its 12-credit contradiction trap) is unnecessary work. Also re-check: aspect-label accuracy, thin-band inflation, cost-preflight accuracy. | Gate 3 + Phase 4 | `20 - Background/Engine-agnostic/Generative art pipelines are a supply chain, not a tool.md` |
...
- [ ] **2.1** **Every duration is an integer count of 60 Hz ticks; every distance is pixels.** Never a
...
- [ ] **2.2** **Number the steps inside `tick()` and declare the numbering authoritative.** A step
...
- [ ] **2.3** **Seed your own RNG** (xorshift32 or similar), sample once per tick inside the fixed
...
- [ ] **2.4** **The input snapshot handed to a multi-tick batch must be a mutable working copy**, and
...
- [ ] **2.5** **Never reconstruct an *edge* from a frame-to-frame state comparison** — a whole 15-tick
...
- [ ] **2.6** **Give every state exactly one door.** If entering a state requires bookkeeping (clearing
...
- [ ] **2.7** **A test for a temporal invariant must span the time the invariant is about.** Ten tests
...
- [ ] **2.8** **Derive a test's expected value from the live knob, and bracket it with a floor *and* a
...
- [ ] **2.9** **A per-tick probability is not a behaviour.** Re-rolling a movement decision every tick
...
- [ ] **2.10** **Collision boxes authored in local space**, one stated convention (`+x` forward,
...
- [ ] **2.11** **Scale art and collision geometry in one place**; make `scale` a **required constructor
...
- [ ] **2.12** **Pull render *decisions* out of scenes into engine-free modules.** Rule: *"if a scene
...
- [ ] **2.13** Playground knob-sweep verification — see **A6**.
- [ ] **2.14** **Use the engine's *discrete* integrator to compute a jump apex.** A review caught a
