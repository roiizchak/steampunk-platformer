...
| 11 | Welcome screen and volume repair | [phase-11-welcome.md](prd/phase-11-welcome.md) | 8 (the level menu it routes into), 10 | 🔨 **IN REVIEW 2026-08-28**, branch `phase-11-welcome`. Not merged. Two tasks. **The volume bug's root cause was proved by measurement, not inferred**: Phaser indexes registered keys by the legacy `event.keyCode`, which a layout may reassign for PUNCTUATION but not for LETTERS — so a press carrying the WRONG `code` with keyCode 219 changed the volume while the RIGHT `code` carrying keyCode 186 did nothing. That is the reported `M`-works/brackets-dead split exactly, and every existing audio spec was green because it only ever pressed a US-layout key. Now dispatched on `event.code`. **The welcome screen is a parallel overlay that PAUSES `Game`**, not a scene boot routes into: `ready` is published only by `GameScene.create()` and 32 spec files assert `sceneKey === 'Game'`. Pausing also closes the ESC/DEV-key leak and the SPACE-jump latch for free, because `Systems.canInput()` puts PAUSED outside its accepted range. Codex plan review converged in **5 rounds** (22 findings, all applied). Gates watched red before being trusted: 5 failed / 6 passed on the un-fixed keys, 5 failed / 6 passed without the pause. **Criteria 11.2, 11.4, 11.6, 11.7 and 11.11 are `play`-owned and UNRUN until the owner plays it on the keyboard that broke.** See [qa/phase-11-welcome.md](qa/phase-11-welcome.md) |
