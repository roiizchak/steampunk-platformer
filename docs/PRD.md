...
| 4 | fal art production + Character Gym | [phase-04-art.md](prd/phase-04-art.md) | **3** (grid size) | ⚠️ **merged 2026-08-09 with known debt** — approved and merged while **reported failing**. Open: **4.27** only — ⚠️ **and no longer for the reason this row gave until 2026-08-25.** It read *"needs a pre-generation anchor gate"*; that gate was written in Phase 5 and exists as `tools/gen/anchorGate.mjs` (**G1 — anchor contact geometry**, whose header names 4.27 outright), and it caught a real anchor defect on the first new art it saw. What is missing is the **wiring**: it is a standalone CLI that `sheetGates.mjs` only mentions in a comment and no `assets:*` script invokes, so nothing re-runs it. **4.27 is open on the pipeline, not on the gate.** 4.10/4.12 closed in Phase 5, **4.16 closed in Phase 6**, **4.2b closed 2026-08-16 by owner amendment** — ceiling raised to $50. Triage: [qa/phase-06-hud.md §Session 2](qa/phase-06-hud.md) |
