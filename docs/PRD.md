...
| 10 | Build and ship | [phase-10-ship.md](prd/phase-10-ship.md) | everything | ✅ **DONE 2026-08-27 — 15 of 15 criteria PASS, live in production.** [steampunk-platformer-2n08tumsc-rois-projects-f9d9895d.vercel.app](https://steampunk-platformer-2n08tumsc-rois-projects-f9d9895d.vercel.app). **10.6 closed against a real Vercel edge response** — all five declared headers on four paths including a 404, every quoted CSP keyword intact. **10.12 closed by the owner playing all five levels to the exit**, which also closed **2.8**, carried open since Phase 2. The phase also closed **4.27** (wired, red-proved, at the spend point) and **4.12**, and dispositioned **4.10** as superseded. 🔴 **Two defects were found by PLAYING the shipped build and by nothing else**: `FOLLOW_LERP` was applied per RENDERED frame, making the camera 4x less responsive at 60 Hz than on the 240 Hz box it was tuned on; and `pixelArt` never governed the canvas→screen resample, so a fractional window scale dropped pixel columns whose positions moved as the world scrolled. Both reported in the same five words. **The general lesson: anything the ENGINE applies per rendered frame, or per presented pixel, sits outside this project's tick rule — which is written about `src/sim/`. The principle was never narrower than the rule; the rule's wording was.** ⚠️ **A bare `vercel deploy` targets PRODUCTION on this project** (no git integration) — always `--target=preview`. ⚠️ `.vercelignore` uses gitignore syntax, and an unanchored `assets/` removed the game's ART from the build box while every local gate stayed green. Both Codex reviews ran: the plan review **did not converge** (5 rounds, all REVISE, at `MAX_ROUNDS`); the implementation review returned **REVISE with 2 CRITICAL** on a diff that had already passed six agents and twelve briefs — one broke the dev-seam gate, which was a pad-able count. **Outstanding, non-blocking:** the rollback is confirmed but NOT rehearsed. Everything in [qa/phase-10-ship.md](qa/phase-10-ship.md), including the §7 ten-phase retrospective and the QA gate's own recorded defect (its worktrees were created at `main`). |

| 11 | Welcome screen and volume repair | [phase-11-welcome.md](prd/phase-11-welcome.md) | 8 (the level menu it routes into), 10 | ✅ **DONE 2026-08-29**, merged from `phase-11-welcome`. Two tasks and a defect the owner found by playing. **The volume bug's root cause was proved by measurement, not inferred**: Phaser indexes registered keys by the legacy `event.keyCode`, which a layout may reassign for PUNCTUATION but not for LETTERS — so a press carrying the WRONG `code` with keyCode 219 changed the volume while the RIGHT `code` carrying keyCode 186 did nothing. That is the reported `M`-works/brackets-dead split exactly, and every existing audio spec was green because it only ever pressed a US-layout key. Now dispatched on `event.code`, owner-confirmed on **both** the Hebrew and the English layout. **The welcome screen is a parallel overlay that PAUSES `Game`**, not a scene boot routes into: `ready` is published only by `GameScene.create()` and 32 spec files assert `sceneKey === 'Game'`. Pausing also closes the ESC/DEV-key leak and the SPACE-jump latch for free, because `Systems.canInput()` puts PAUSED outside its accepted range. Redesigned mid-phase on the owner's word to a single static fal plate under a full-width band, with ENTER to the level menu as the only way in. 🔴 **And the owner's playtest broke 11.4**: at volume 50 the game was barely audible. `MIX_DB` was applied to peak-normalised WAV cues but to raw OGG beds — one column, two units — and with a 19 dB crest factor the beds shipped ~25 dB under. Beds now mix by measured RMS (**+12.05 dB**); the gate that was green throughout is replaced, not re-bounded. Codex plan review converged in **5 rounds** (22 findings), the implementation review in **5 more** (10 findings), all applied. See [qa/phase-11-welcome.md](qa/phase-11-welcome.md) |
| 12 | Touch and responsive support | [phase-12-touch.md](prd/phase-12-touch.md) | 8 (the level menu it adds a touch route to), 11 (the welcome screen it must be tappable through) | ✅ **DONE 2026-09-03 — 24 of 25 criteria PASS, and the 25th is ACCEPTED OPEN by owner decision.** Merged 2026-09-01 (`9eba673`), closed out on `phase-12-closeout` and live in production. 🔴 **CLAUDE.md §3 says a phase with a failing criterion is reported failing; the owner overrode that explicitly for 12.23 and the override is recorded rather than dissolved — this phase ships by decision, not by convergence.** 12.13, 12.14 and 12.14b closed on the owner's device against the branch preview; 12.19 and 12.21 closed by this session. 22 of 25 PASS (12.14b was added as a row that had never existed). **12.19 and 12.21 now PASS**; **12.23 is NOT MET at the owner's three-round cap and is ACCEPTED OPEN by owner decision 2026-09-03** — the phase is accepted with the criterion open rather than the cap lifted, which is a decision and not a pass; — rounds 21, 22 and 23 all returned `VERDICT: REVISE`, every finding applied, and each round found a real defect rather than stalling; **12.13, 12.14 and 12.14b PASS on the owner's device 2026-09-03** — four gestures, six glyphs and two hazard plates in one trip, reported as *"I played and it worked as expected"*; one sentence for three criteria, recorded as what it is. **`PLATE_ALPHA` stays at 0.9**, so the number does not come back down. 12.14 has no automated cover and is not getting any — the contrast gate was deleted, not failed, and rebuilding it after seeing the art is the post-data selection this phase kept catching. Close-out regression: unit **3146/3146**, sim-isolated 3133+13 skipped, e2e **236/236** across all seven projects, build + `verify-dist` ok. 🔴 **Round 23 is the one worth remembering**: three mutations in a row (M115, M117, M118) each defeated a gate over the SHAPE of the audition template's concatenation, the last with `.slice(0, 0)` on a real read — **a regex cannot tell a read from a discarded read**, so the concatenation moved to `tools/gen/auditionDocument.mjs` where a test just runs it. The matrix carries **122 rows**, a figure this session had to correct three times. Original state below. **12.24 closed 2026-09-02**: three preview rounds on the owner's phone, each producing specific defects against screens reached by touch. 🔴 **Still open, and each for its own reason — see [qa/phase-12-touch.md](qa/phase-12-touch.md):** **12.13** the gesture checks themselves (drag off the edge, pinch, double-tap) have never been run, though both rotate-gate defects the hands-on passes DID find are fixed with e2e reproductions watched red · **12.14** the per-stroke contrast gate is **deleted, not failing** — the 2026-08-31 plate redesign put verdigris in every recess, so the ink mask stopped being the mark and read 12–37 "strokes" per face where six had 1–4; rebuilding the statistic after seeing the art it would judge is the post-data selection this phase kept catching, so the criterion rests on a hands-on pass and `ui-ux-tester`, neither of which has run against these bytes · **12.19** thirteen of the 92 mutation rows red nothing · **12.23** seven Codex implementation rounds (14–20) all returned `VERDICT: REVISE` with every finding applied, and the row's own standard is that an executed review with an unresolved verdict is NOT MET. The mobile-polish session's review returned REVISE too. **A phase with an open criterion is not done — CLAUDE.md §3**, and the owner ruled it shipped rather than clean. Phase 13 built on it and is done. On-screen controls, multi-touch, and a responsive layout so the game is playable on a phone. **The Codex plan review's first blocker was that a touch player cannot START the game**: all three terminal screens are keyboard-only (`TitleScene.ts:333-335`, `LevelSelectScene.ts:145-167`, `gameComplete.ts:119-135`), so in-play controls alone would have shipped a phone build nobody could reach — the three screens are in scope as a consequence of the goal. The design rests on one measurement: `FIT` holds the backing store at 1920×1080 at every viewport, so a 160 game px button is **55.6 CSS px** at the worst in-scope viewport (iPhone SE landscape, scale 0.347) and **32.5 CSS px** at phone portrait (0.203), where a legal 44 px target would cost a fifth of the play area per button — hence a rotate prompt for phone portrait, with tablet portrait playable. Plan review converged at round 4 of 5: **24 findings, 5 BLOCKER, every one applied**. Two more would each have cost a build — `attachHud` runs BEFORE `UIScene.create()`, and a hardcoded plate dimension was the inference `FAL-MODELS.md:115-122` forbids, written twice. Spend against the new `$5` touch-UI ceiling. See [reviews/phase-12-touch-plan.md](reviews/phase-12-touch-plan.md) |
| 13 | Mobile polish and the filled view | [qa/session-mobile-polish.md](qa/session-mobile-polish.md) + [round 2 & 3](qa/session-mobile-polish-02-device-report.md) | 12 | ✅ **DONE 2026-09-02**, merged (`5447c8a`) and live at [steampunk-platformer-jet.vercel.app](https://steampunk-platformer-jet.vercel.app). Six items the owner reported from a real device, then three further device rounds. ⚠️ **No numbered gate of its own** — it was scoped as a polish pass on shipped work, not a phase, so "done" here means every reported item fixed and mutation-proved, not 24 criteria closed. **Item 6 shipped TWICE**: `Phaser.Scale.EXPAND`'s only clamp is `scale.min`/`max`, which `ScaleManager` feeds to `displaySize` — the CSS style size — so it drew a **1920 CSS px** canvas into a 1040 px viewport; `FIT` stays and `src/game/viewSize.ts` makes the game size a function of the viewport. **The world contract moved with it**: 1920×1080 is now the design view and the guaranteed minimum, height pinned, width up to `MAX_GAME_WIDTH`. 🔴 **Three defects were invisible to gates that all passed, and each names the reason.** The touch controls sat 19–24 CSS px from the edge, inside Android's back-gesture strip, while every assertion about them was in GAME pixels. The gear counter was wrong three times because `TextMetrics.fontSize` is `ascent + descent` — the box height, not the font size — and Phaser measures the style's test string `|MÉqgy`, not the digits; **M129 reproduces the owner's photograph to the tenth of a pixel**, 92.4 against 88.0. And removing the welcome screen's audio line was a LAYOUT change: the four row fractions two adversarial briefs tuned are indexed positionally, so three rows on the first three give margins of 0.120 against 0.211. M95–M134 all applied and reverted with C12 verification. Final: unit **3126/0**, sim-isolated 3113+13 skipped, e2e **229/0** across six projects, build + verify-dist ok. **Outstanding:** the hazard-visibility pass item 3's abandoned occlusion bound owes — the level-01 sentry behind the pause plate, the level-04 goal under the jump plate. |
...
- **Every generation logged** to [GENERATION-LOG.md](GENERATION-LOG.md): model, prompt, seed, cost,
  path, kept/discarded.
- **The fal art-spend ceiling is `$60`**, raised from `$55` by owner decision on **2026-08-29**,
  which also settled the `$55.50` overrun `GENERATION-LOG.md` had recorded as unresolved for three
  sessions — every earlier overrun was cleared by an explicit decision but **no replacement figure
  had ever been named**, and a ceiling a document invents is a limit nobody set. It was raised from
  `$25` by owner amendment on 2026-08-16 after
...
- **The fal touch-UI-spend ceiling is `$5`**, set by owner decision on **2026-08-29 before Phase
  12's first generation** — the ordering Phase 7 established and the half of 4.2b no ceiling alone
