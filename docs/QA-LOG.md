...
| 9 — Polish, juice, particles | [qa/phase-09-polish.md](qa/phase-09-polish.md) | [prd/phase-09-polish.md](prd/phase-09-polish.md) | [plan](reviews/phase-09-plan.md) · [impl](reviews/phase-09-impl.md) |
| 10 — Build and ship | [qa/phase-10-ship.md](qa/phase-10-ship.md) | [prd/phase-10-ship.md](prd/phase-10-ship.md) | [plan](reviews/phase-10-plan.md) · [impl](reviews/phase-10-impl.md) |
| 11 — Welcome screen and volume repair | [qa/phase-11-welcome.md](qa/phase-11-welcome.md) | [prd/phase-11-welcome.md](prd/phase-11-welcome.md) | [plan](reviews/phase-11-plan.md) · [impl](reviews/phase-11-impl.md) |
| 12 — Touch and responsive support | [qa/phase-12-touch.md](qa/phase-12-touch.md) | [prd/phase-12-touch.md](prd/phase-12-touch.md) | [plan](reviews/phase-12-touch-plan.md) · [impl](reviews/phase-12-touch-impl.md) | plan ✅ **APPROVED at round 4 of 5, 24 findings, 5 BLOCKER, all applied** · impl 🔴 **23 rounds, every one REVISE, every finding applied** — the row does not close on that standard. ⚠️ **This index had NO row for phase 12 until 2026-09-03**, through the whole phase and two close-out sessions: `docs-contract.test.ts` checks the per-phase LOG against the PRD roster, and nothing checks this table. ✅ **DONE 2026-09-03, 24 of 25 PASS**, the 25th (12.23) **accepted open by owner decision** rather than converged. 12.13, 12.14 and 12.14b closed on the owner's device in one trip against a branch preview. Close-out: unit **3146/3146**, sim-isolated 3133+13 skipped, e2e **236/236** across seven projects, build + `verify-dist` ok. |
...
| — Tier-5 false gates, gate holes, the capture round (not a phase) | [qa/session-tier5-gate-holes.md](qa/session-tier5-gate-holes.md) + [-02-tweens](qa/session-tier5-gate-holes-02-tweens.md) + [-03-sweep](qa/session-tier5-gate-holes-03-sweep.md) + [-04-briefs](qa/session-tier5-gate-holes-04-briefs.md) + [-05-reshoot](qa/session-tier5-gate-holes-05-reshoot.md) | [plan](reviews/session-tier5-gate-holes-plan.md) · [impl](reviews/session-tier5-gate-holes-impl.md) | plan ✅ **three rounds, REVISE each** · impl ✅ **BLOCK, 11 findings, all 11 applied** — incl. a 9.3 teardown rule that stayed GREEN on a real violation. Full sweep green after it: e2e 136 + 109, unit 169/2541, sim-isolated 2538 + 3 skipped, build ok |
| — The HUD banner's placement + the pit rule (not a phase) | [qa/session-hud-and-pits.md](qa/session-hud-and-pits.md) + [gate owners](qa/session-hud-and-pits-02-gate-owners.md) + [hazard clearance](qa/session-hud-and-pits-03-hazard-clearance.md) + [flush seams](qa/session-hud-and-pits-04-flush-seams.md)  Then a FIFTH report of the same sentence, and this one was **reproduced before it was fixed**: `01a518e`'s platform widening made two slabs abut exactly, and two co-planar touching solids draw as one platform and collide as two. Seam census: `main` 0, the branch **2** — in levels 2 and 3, the exact two levels reported. Confirmed on screen by the owner at `?pin=1`, feet `(8190,1632)` and `(10686,1536)`, `cause=geometry`. Fixed by merging the strips at build time (`tools/gen/mergeStrips.mjs`); the resolver latch underneath is a **tracked latent defect**, deliberately not fixed. Regression after: unit **2735/0**, sim-isolated 2731 + 4 skipped, e2e **150**, build ok | [handoff](HANDOFF.md) | [plan](reviews/session-hud-and-pits-plan.md) · [impl](reviews/session-hud-and-pits-impl.md) | plan ✅ **two rounds REVISE, both applied; round 3 aborted on a usage limit and re-run against the IMPLEMENTATION instead** · impl — see the review file. Two owner-reported defects from playing the shipped build: the controls legend drew full-width across the level, and four of the five walled pits could be fallen into for free. Regression after: unit **2690/0**, sim-isolated 2686 + 4 skipped, e2e **148**, build ok. Then a THIRD owner defect from playing — *"there is a hazard that is not being seen"* — fixed with one $0.15 fal generation composited into tileset cell 12, which **breaches the $55 art ceiling to $55.20** (cleared by the owner; no new ceiling set). Then a FOURTH — the same sentence again, *"I still get stuck by a hazard that I cannot see"* — and this one was **geometry, not art**: five floor hazard runs left exactly 96 px (one tile) to the wall facing them against a 132 px player, so there was nowhere to stand. New rule in `tools/gen/hazardClearance.mjs`, gated over the shipped bytes AND enforced in the generator, with 8 synthetic red proofs. Regression after: unit **2705/0**, sim-isolated 2701 + 4 skipped, e2e **148**, build ok |
| — **The iPhone black screen** (not a phase) | [qa/session-webkit-audio.md](qa/session-webkit-audio.md) | [handoff](HANDOFF.md) | — | 🔴 **Shipped to production and found by the owner's FRIENDS, not by 229 green e2e tests.** Safari decodes no Ogg container; both music beds were `.ogg` with a single url, so `verifyAudio` found neither in the cache and `refuseToRoute` put a **BOOT REFUSED** screen on every iPhone — every browser on iOS is WebKit. **The gate was right; nothing could see it.** All six Playwright projects were Chromium, which decodes Ogg: a substrate assumption nobody wrote down, and one a suite cannot test because it is standing on it. Fixed with `altUrls` alternates the browser chooses between (AAC 128k, 1.9 MB each; only the chosen file is fetched, so Chromium downloads nothing new), a catalog rule that refuses an Ogg with no alternate **and** an alternate that is itself an Ogg — *"must gain a browser"*, not *"must have a fallback"* — and a seventh **`webkit`** Playwright project. 🔴 **The first version of that spec was DECORATION and only the mutation found it**: it read `document.body.innerText` for `BOOT REFUSED`, which `refuseToRoute` draws onto the CANVAS, so M136 came back *2 passed* against a build that refused on every load. Replaced with the `console.error` the refusal actually emits plus a 400 kB drawn-frame floor (healthy 1,978,349 bytes vs refused 62,168 — a 32× gap). M135–M139 all proved. Final: unit **3137/0** in 221 files, e2e **231/0** across seven projects, build + verify-dist ok (14 audio files) |
| — Six mobile polish items the owner reported from a real device (not a phase) | [qa/session-mobile-polish.md](qa/session-mobile-polish.md) + [the padlock](generations/phase-13-padlock.md) | [handoff](HANDOFF.md) | [impl](reviews/session-mobile-polish-impl.md) | plan ✅ **five Codex rounds before approval at the owner's instruction — REVISE ×4 then APPROVED, 33 findings, all applied** · impl — see the review file. 🔴 **Item 6 shipped TWICE.** `Phaser.Scale.EXPAND` landed and was then replaced: its only clamp is `scale.min`/`max`, which `ScaleManager` feeds to `displaySize` — the CSS style size — so bounding the view also bounds the canvas in CSS pixels, and it drew a **1920 CSS px** canvas into a 1040 px viewport. Caught by the live-resize gate on its FIRST run; a fresh-boot spec at each size would have seen nothing. `FIT` stays and `src/game/viewSize.ts` makes the game size a function of the viewport instead. Every consumer landed for EXPAND — shake camera, parallax attachment, tap routes, level guards — was unchanged by the reversal; the expensive part was **35 lines of prose across 24 files** that named the old mechanism. 🔴 Two gates were DECORATION when first written and were found by running the mutation they name: `toContain('keepTapRoutesSized(')` survives a rename to `NOT_CALLED_…`, and the "never calls `setText`" case was unfalsifiable because neither the interface nor the fake carried `setText`. 🔴 Item 3 **abandons** the touch-plate occlusion criterion rather than weakening it — 0.9 leaves 22 % residual against a 60 % rule, past the 0.86 measured to erase content — on owner authority, with a hands-on hazard-visibility pass replacing it that is **still owed**. 🔴 `GENERATION-LOG.md`'s running total was stale by $2.10 and is corrected to $57.75 of $60. Regression after: unit **3109/0** in 219 files, e2e **227/0** across all six projects, typecheck clean, build + verify-dist ok. Then the **implementation review: 6 findings, all 6 applied, none rejected** — including a source-text gate that passed with the code it names COMMENTED OUT, and a per-frame `scale.refresh()` emitting a global RESIZE 60x a second. Final: unit **3116/220 files**, e2e **227/0**. 🔴 **Then a Vercel preview went to the phone and two more reports came back** — [round 2](qa/session-mobile-polish-02-device-report.md): the touch controls sat **19–24 CSS px** from the edge, inside Android's back-gesture strip, invisible to a whole test file whose every assertion was in GAME pixels (`TOUCH_EDGE_PX` 64 → 128 by owner decision, with a CSS floor added); and the gear count was wrong for the **third** time, 4.4 px low, because `TextMetrics.fontSize` is `ascent + descent` — the box height, not the font size — and Phaser measures the style's test string `|MÉqgy` rather than the digits. Replaced with a two-measurement ink placement in a new `src/render/counterInk.ts`. **M129 reproduces the owner's photograph to the tenth of a pixel** (`digits at 92.4, icon at 88.0`) from a gate that is green on the repair. Regression: unit **3121/0**, sim-isolated 3108+13 skipped, e2e **228/0**, build + verify-dist ok. 🔴 **Round 3, same file**: the welcome screen still advertised `M mute · [ / ] volume` on a phone — two KEYBOARD keys a phone does not have, on the one screen whose docstring argues at length that a screen advertising a control owes the player its value. Desktop-only now. **The removal is a layout change, not a deletion**: `applyLayout` places items by INDEX against a four-entry table two adversarial briefs tuned, so three rows on the first three fractions give margins of 0.120 against 0.211 — the *"a row has gone missing"* defect those briefs were run to find. `titleRowSpread(n)` re-derives the same equal-gap, equal-optical-margin solve for n rows; `titleRows(4)` still returns the tuned literal. 🔴 **That early return made its own gate decoration** and M133 left it green — the derivation is a separate export so the four-row agreement case can go red *(C2)*. M130–M134 all proved, including M131: a row created EMPTY holds the spacing open, and only asserting the drawn COUNT tells that from a row not created. Final: unit **3126/0**, sim-isolated 3113+13 skipped, e2e **229/0**, build ok |
| — **The tab icon and the link preview** (not a phase) | [qa/session-page-identity.md](qa/session-page-identity.md) | [handoff](HANDOFF.md) | — | One owner request after Phase 12 shipped. 🔴 **The page had no favicon and no description, and never had**: thirteen phases of tabs showing the browser's default globe and shared links arriving as bare URLs — and this build has gone to friends more than once. The icon is a downscale of the game's own collectible gear (no new art, `$2.25` left on the ceiling); the card is a real production frame of level-01. 🔴 **The first cover was captured off the DEV build** and carried the two dev-only scene shortcuts in its banner — it would have put in every shared link exactly the prose `verify-dist` refuses to put in the bundle — **and then the comment explaining that named them and the gate red on the build.** Six mutations, N1–N8; **N3 and N5 were GREEN on the shipped half** until it was made to compare the three description tags to each other and read the cover's IHDR against the declared size. Two more gates fired unprompted and were right: the 400-line rule (a seventh split rather than an exemption) and the LF sweep, which reported `favicon.ico` as a text file the hour it arrived — its denylist working exactly as its header argues. unit **3152/3152**, sim-isolated 3139+13 skipped, e2e **236/236**, build + `verify-dist` ok. |
