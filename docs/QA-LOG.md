...
| — The HUD banner's placement + the pit rule (not a phase) | [qa/session-hud-and-pits.md](qa/session-hud-and-pits.md) + [gate owners](qa/session-hud-and-pits-02-gate-owners.md) | [handoff](HANDOFF.md) | [plan](reviews/session-hud-and-pits-plan.md) · [impl](reviews/session-hud-and-pits-impl.md) | plan ✅ **two rounds REVISE, both applied; round 3 aborted on a usage limit and re-run against the IMPLEMENTATION instead** · impl — see the review file. Two owner-reported defects from playing the shipped build: the controls legend drew full-width across the level, and four of the five walled pits could be fallen into for free. Regression after: unit **2690/0**, sim-isolated 2686 + 4 skipped, e2e **148**, build ok. Then a THIRD owner defect from playing — *"there is a hazard that is not being seen"* — fixed with one $0.15 fal generation composited into tileset cell 12, which **breaches the $55 art ceiling to $55.20** |
