...
| [generations/phase-12-touch-plate.md](generations/phase-12-touch-plate.md) | Phase 12 — the touch-control plate. **Two takes, NEITHER adopted**: six buttons then seven, both times the model filling space the prompt left it. Settled that `nano-banana-pro` at `1:1`/`2K` returns **2048 × 2048** — previously UNMEASURED — which confirms `2048 % 3 = 2` and that the centre-crop step is load-bearing. Game ships the grey-box faces | `fal-ai/nano-banana-pro` | 2 · $0.30 |
