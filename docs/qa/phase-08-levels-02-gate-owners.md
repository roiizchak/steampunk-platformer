...
| Finding | Disposition |
|---|---|
| 🔴 **8.7 read only `workMedianMs`. GPU time was sampled and discarded** — so the one cost a denser level actually incurs was unmeasured. Level-05 paints **3.7x** the tiles | **APPLIED** — gated. Measures **0.51x** against a 2x bound: the camera cull proved at the rasteriser rather than assumed |
