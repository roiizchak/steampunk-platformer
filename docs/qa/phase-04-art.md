...
**This log reached 557 lines.** On 2026-09-03 the 4.23 post-mortem moved to a flat sibling, per
...
⚠️ **Only the section BELOW the vault-out could move, and the first attempt at this split moved the
wrong one.** `docs-contract.test.ts` uses this log as its regression case for a slice carrying **two**
legitimate criterion tables — the criterion-by-criterion walk and the later summary — so both must
stay between the phase heading and the vault-out heading. Moving the walk out turned that test red
with *"4.2b should appear twice in the SECTION: expected 0 to be greater than 1"*.

| Part | What is in it |
|---|---|
| [02 — criterion 4.23, red on `main` after the phase closed](phase-04-art-02-criterion-4-23.md) | the 2026-08-17 post-mortem: what was measured in the running game, the mechanism, and the red proof that caught a bug in its own fix |
