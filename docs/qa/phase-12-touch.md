...
**Derived from the table above, 2026-09-03:** 12.19 PASS. 12.21 PASS, after being found falsely so.
12.13's machine half is built and red-proved; 12.14's agent half is MET, both briefs run.
**12.23 is NOT MET — round 21 ran and returned `VERDICT: REVISE`. 12.13, 12.14 and 12.14b all await
...
119 today.** 93 of the original ids were plain numeric, which is the likely source of the off-by-one; the
...
### The pinch case cannot go red on Chromium, and is recorded rather than counted
...
*A gate that cannot go red is decoration (C2)*, so it is not counted as evidence for 12.13. It stays
for two narrower reasons, both stated in the file: it proves the six-move pinch does not crash, drop
the contacts or stop the tick, and it is a tripwire for the day a future Playwright or Chromium does
synthesise gestures. **The pinch claim's real evidence is the CSS gates — which DO red, M105 and
M106 — and the device.** Written down rather than left as a green tick nobody had questioned.
