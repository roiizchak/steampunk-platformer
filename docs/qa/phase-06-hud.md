...
> criterion 5.11's existing spec provides no evidence about Phase 6's cost *by construction* — it is
> a ratio between two halves of one page that vary only in enemy count, so any constant per-frame
> cost divides out to ~1×. A HUD-vs-no-HUD interleaved A/B is owed and has not been run.
>
> This log said **"6.9 PASS, with an open coverage gap"** until Codex's implementation review called
> it a blocker (C2), and it was right: a verdict that contradicts its own evidence one line below is
> exactly what the second review exists to catch. *A phase with a failing or unrun criterion is
> reported failing.* The remaining owed work is listed at the end.
...
|---|---|---|---|
| **6.1** | Pickup increments the counter; tabular figures | e2e | **PASS** (tween trajectory not asserted — see owed) |
| **6.2** | HUD pinned under pan **and** zoom | `qa-expert` | **PASS**, one assertion still a flag read — see owed |
| **6.3** | Built from live game size; resize does not crop | `qa-expert` | **PASS, with a stated limit** |
...
| **6.9** | ≤400 lines; diff reviewed; adversarial; frame budget | `code-reviewer` ×2 + `performance-engineer` | 🔴 **UNRUN — frame budget not measured** |
| **6.10** | Codex plan review applied/recorded | — | **PASS — 8 findings, 8 applied** |
| **6.11** | Codex implementation review applied/recorded | codex | **RAN — returned BLOCK.** 3 applied, 1 partial, 1 recorded. [phase-06-impl.md](../reviews/phase-06-impl.md) |
...
## 🔴 Owed before Phase 6 can be reported done

Every item below is a gate owner's or Codex's finding that was **not** closed. None is a nice-to-have.
...
### Brief 2 coverage, stated honestly
