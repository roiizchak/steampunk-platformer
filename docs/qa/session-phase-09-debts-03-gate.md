...
| S3-4 (a) | high | `this.add.tween(config)` is a real Phaser 4.2.1 entry point (`phaser.d.ts:26869`, `:28201`) and every rule keys on the object being literally named `tweens`. Bypasses **SIX** rules at once — 9.3b, 9.3c, **9.3d**, 9.2, 9.2b and 9.2c. ⚠️ This row originally said *four*, omitting 9.3c's `OPENS_A_TWEEN` filter and 9.3d (bypassed in what is now `tweenIdentity.ts`, extracted from `tweenCallbacks.ts:180` under the 400-line rule); `SESSION-PROMPT-next.md:66` had it right and this did not. | 🔴 **CLOSED 2026-08-25**, superseding *"RECORDED as D14, with the ABSENCE PINNED"*. The blocker was smaller than this row judged: `tween` is unique to the factory/creator (the manager has no `.tween()`), so recognition is a name test like the manager's, and the cost was fixtures rather than machinery. `namesSceneFactory` requires a **member** access (`this.add`, `scene.add`) and refuses a bare `add` — deliberately stricter than `namesTweenManager`, because `add` is an ordinary word and the scanner is name-based, so a bare match would false-red any unrelated object called `add`. The absence pin is deleted and `tests/unit/tween-add-factory.test.ts` replaces it, carrying that acceptance fixture. See `session-tier5-gate-holes-02-tweens.md` §Batch 4. |
