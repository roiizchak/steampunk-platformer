...
**Continues in [session-tier5-gate-holes-02-tweens.md](session-tier5-gate-holes-02-tweens.md)** — Batch 4 onward. Flat sibling per CLAUDE.md §6; `tests/unit/file-size.test.ts` globs `docs/qa/*.md` non-recursively, so a subdirectory would silently leave the split half ungated.
