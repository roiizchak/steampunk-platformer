...
| 9.5 | Frame budget holds under worst case | `performance-engineer` ×2 | ❌ **FAILING — owner decision owed. This row read "PASS, qualified" for one hour and the Codex implementation review blocked it, correctly.** The criterion says *max enemies + max particles + shake*. The measurement carries **no combat** — `installStorm` holds the player invulnerable in every arm — and the **smallest** of four shakes; entries 43 and 44 say so and concede the defence is *"an argument and not a measurement"*. **A caveat cannot reverse the criterion's wording.** What IS established: every defect the two prior FAILs named is fixed on disk, and **E6** re-proved the shake-load guard on the current post-`8c9d0fc` tree. What is not: the stated combined worst case has never been measured. Three ways out, and all three are the owner's: measure combat + a worst applicable shake; amend 9.5's wording by explicit owner amendment; or leave it failing. See §*"The close round — 2026-08-24"* (D5).|
