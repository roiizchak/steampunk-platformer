...
| — | LOW | Every `GameScene.create()` restarts both beds from zero, so a dev scene toggle cuts the music. 7.5 counts tracks, not continuity. | ✅ **CLOSED 2026-08-23** *(session inventory 1b.2)*. Was **RECORDED** — *"no level transition exists yet; it becomes real in Phase 8"* — and **Phase 8 shipped five levels and transitions**, so the reason expired by its own terms and nobody re-read it. `createAudio` now RETIRES its predecessor and adopts the still-looping beds rather than removing and restarting them. 7.5 is preserved: `bedsToStart` returns only keys catalogued AND not already playing, so the live set stays at one of each. Gate: `tests/unit/audio-beds.test.ts` — `createAudio` had no unit test at all before it. |
